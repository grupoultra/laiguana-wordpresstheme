
<div class="col-g articulo">
    <div class="row">
        <div class="col-xs-12 title-span  margin-top-10">
            <div class="banner-c banner728x90 hidden-mobile">
                <img class="clean img-responsive" src="<?php echo get_bloginfo('template_url') ?>/assets/img/comentarios.jpg" style="padding: 0; margin: 0;" alt="La Iguana comentarios">
            </div>
            <div class="margin-bottom-10 banner-c banner320x50 hidden-desktop">
                <img class="clean img-responsive" src="<?php echo get_bloginfo('template_url') ?>/assets/img/comentariosm.jpg" style="padding: 0; margin: 0;" alt="La Iguana comentarios">
            </div>
            <div class="fb-comments" data-href="<?php the_permalink(); ?>"
                 data-width="100%" data-numposts="10" >
            </div>
        </div>
    </div>
</div>
